...
So your visibility form should be:
...
Why this fixes it
You currently have this in the delete form:

value="<?= (
    int)$event['id']
?>"

Although PHP can sometimes handle multiline expressions, your file is evidently being parsed in a way where the transition back into HTML/PHP is causing the parser to interpret $event incorrectly.

Also, throughout the file you have several unnecessarily complicated constructions such as:
...
These aren't inherently invalid, but they make this file much harder to debug.

You can safely simplify them to:
...
One important thing
The error says line 2840, but the actual syntax problem can be several lines before that. PHP often reports the location where it finally becomes impossible to continue parsing, rather than the exact place where the mistake began.

Your supplied code also has this pattern:

value="<?= (
    int)$event['id']
?>"

I would change every occurrence of that pattern to:

value="<?= (int) $event['id'] ?>"

That should address the $event parse error you're seeing.
...
ChatGPT said:
Absolutely. Below is the whole corrected admin-events.php, keeping your existing design and functionality, but simplifying the PHP/HTML expressions that are causing the $event parse error.
